...
def sync_gradients_gather_scatter(model, world_size):
    ...
    for param in model.parameters():
        ...
        if rank == 0:
            ...
            dist.gather(grad, gather_list=gathered_grads, dst=0)
            averaged_grad = torch.zeros_like(grad)
            for gathered_grad in gathered_grads:
                averaged_grad.add_(gathered_grad)
...
def main():
    ...
    if args.local_rank != -1:
        if args.master_ip is None or args.master_port is None:
            raise ValueError("Distributed training requires --master_ip and --master_port.")
